...
storageDir = "../../data/ookla-speedtest/raw"
...
def sort_files_and_directories(contents, files={}):
    for entry in contents:
        ...
        if (
            entry["type"] == "file"
            and entry["name"].find("headers") == -1
            and "_20" in entry["name"]
        ):
            dataset = entry["name"]  # [:entry['name'].index('_20')]
            files[dataset] = {
                "name": entry["name"],
                "url": entry["url"],
                "age": entry["mtime"],
...
def download(files):
    if not files:
        print(
            "No data extract files found. If this is an error, please contact your technical account manager."
        )
        return

    for data_set, file in files.items():
        r = requests.get(file["url"])
        flocation = file["url"].split("/")[-1]
        dlocation = storageDir + flocation
        # wget.download(file['url'])
        # z = zipfile.ZipFile(io.BytesIO(r.content))
        # flocation = storageDir + '/' + file['name']
        # z.extractall(flocation)
        print(("Downloading: %s" % (file["name"])))
        with open(flocation, "wb") as loc:
            loc.write(r.content)
