  ...
  for(time_type in c("monthly", "annual", "daily")){
    for(country_code in c("syr")){
      ...
      for(file_i in rev(bm_files)){
        ...
        if(time_type == "daily"){
          date_i <- file_i %>%
            str_replace_all("VNP46A2_NearNadir_Composite_Snow_Free_qflag_t", "") %>%
            str_replace_all(".tif", "")
        ...
        dir.create(file.path(ntl_bm_dir, "aggregated_individual_files", paste0("adm", adm_i)))
        dir.create(file.path(ntl_bm_dir, "aggregated_individual_files", paste0("adm", adm_i), time_type))
