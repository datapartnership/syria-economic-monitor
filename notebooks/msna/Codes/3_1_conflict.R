...
safety <- msna %>%
  select(id, starts_with("safety_"), year) %>%
...
bar_chart <- ggplot(safety, aes(x = percentage, y = safety_r, fill = year)) +
  ...
  labs(title = "",
       ...
       y = "Security Concern") +
  scale_x_continuous(limits = c(0, max(safety$percentage) + (max(safety$percentage)/10))) +
  scale_fill_manual(values = bar_colors) +
  theme_minimal() +
  theme(
...
ggsave(here("Output", "bar_sec.png"), bar_chart, width = 10, height = 6)
