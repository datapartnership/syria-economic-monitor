...
def create_three_part_gdp_line_charts(
    ...
    if "GDP" in gdp_column or "gdp" in gdp_column.lower():
        if not gdp_no2.empty:
            r2_no2 = calculate_r_squared(
                gdp_no2[gdp_column].values, gdp_no2[no2_column].values
            )
        if not gdp_ntl.empty:
            r2_ntl = calculate_r_squared(
                gdp_ntl[gdp_column].values, gdp_ntl[ntl_column].values
            )
        if not gdp_evi.empty:
            r2_evi = calculate_r_squared(
                gdp_evi[gdp_column].values, gdp_evi[evi_column].values
            )
    ...
    def create_dual_axis_chart(
        merged_df, indicator_col, indicator_name, title, gdp_color, indicator_color
    ):
        if merged_df.empty:
            # Create empty chart with message
            return (
                alt.Chart(
                    pd.DataFrame({"x": [0], "y": [0], "text": ["No overlapping data"]})
                )
                .mark_text(fontSize=16, color="gray")
                .encode(
        ...
        gdp_line = (
            ...
            .encode(
                ...
                y=alt.Y(
                    f"{gdp_column}:Q", title=y_axis_title, scale=alt.Scale(zero=False)
                ),
                tooltip=[f"{year_column}:O", f"{gdp_column}:Q", f"{indicator_col}:Q"],
            )
            .add_params(legend_selection)
            .transform_calculate(variable=f"'{variable_name}'")
        )
        ...
        indicator_line = (
            ...
            .add_params(legend_selection)
            .transform_calculate(variable=f"'{indicator_name}'")
        )

        # Combine the two lines
        combined = (
            alt.layer(gdp_line, indicator_line)
    ...
    chart1 = create_dual_axis_chart(
        gdp_no2, no2_column, "NO2 Levels", title1, colors[0], colors[1]
    )
    chart2 = create_dual_axis_chart(
        gdp_ntl, ntl_column, "Nighttime Lights", title2, colors[0], colors[2]
    )
    chart3 = create_dual_axis_chart(
        gdp_evi, evi_column, "Vegetation Index", title3, colors[0], colors[3]
    )
...
def create_three_part_gdp_comparison(
    ...
    fig.suptitle(
        "GDP Trends vs Economic/Environmental Indicators in Syria",
        fontsize=16,
        fontweight="bold",
        y=0.95,
    )
    ...
    ax1_twin.set_ylabel(
        "NO2 Levels (molecules/cm²)", color=indicator_colors[0], fontweight="bold"
    )
    ax1.set_title(
        "GDP vs Air Pollution (NO2 Levels)", fontsize=14, fontweight="bold", pad=20
    )
    ...
    ax2_twin.set_ylabel(
        "Nighttime Lights Intensity", color=indicator_colors[1], fontweight="bold"
    )
    ax2.set_title(
        "GDP vs Economic Activity (Nighttime Lights)",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
    ...
    bars6 = ax3_twin.bar(
        ...
        color=indicator_colors[2],
        alpha=0.8,
    )

    ax3.set_ylabel("GDP (Local Currency Units)", color=gdp_color, fontweight="bold")
    ax3_twin.set_ylabel(
        "Enhanced Vegetation Index (EVI)", color=indicator_colors[2], fontweight="bold"
    )
    ax3.set_title(
        "GDP vs Agricultural Activity (Enhanced Vegetation Index)",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
...
def create_normalized_comparison_chart(
    ...
    if not no2_data.empty:
        merged_data = pd.merge(
            merged_data,
            no2_data[[year_column, no2_column]],
            on=year_column,
            how="outer",
        )
    ...
    if not ntl_data.empty:
        merged_data = pd.merge(
            merged_data,
            ntl_data[[year_column, ntl_column]],
            on=year_column,
            how="outer",
        )
    ...
    if not evi_data.empty:
        merged_data = pd.merge(
            merged_data,
            evi_data[[year_column, evi_column]],
            on=year_column,
            how="outer",
        )
    ...
    if gdp_column in merged_data.columns:
        indicators["GDP (Normalized)"] = normalize_data(
            merged_data[gdp_column].dropna()
        )
    if no2_column in merged_data.columns:
        indicators["NO2 (Normalized)"] = normalize_data(
            merged_data[no2_column].dropna()
        )
    if ntl_column in merged_data.columns:
        indicators["NTL (Normalized)"] = normalize_data(
            merged_data[ntl_column].dropna()
        )
    if evi_column in merged_data.columns:
        indicators["EVI (Normalized)"] = normalize_data(
            merged_data[evi_column].dropna()
        )
    ...
    ax.set_title(
        "Normalized Comparison of GDP and Economic Indicators in Syria",
        fontsize=14,
        fontweight="bold",
    )
...
def create_correlation_heatmap(
    ...
    ax.set_title(
        "Correlation Matrix: GDP and Economic Indicators",
        fontsize=14,
        fontweight="bold",
    )
