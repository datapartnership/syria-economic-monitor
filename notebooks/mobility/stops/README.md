...
# Estimating Hubs in Lebanon and Syria through Mobility Data

This pilot study seeks to explore leveraging mobility data to estimate the location of movement hubs and, in particular, estimate the location of formal and informal activity between Lebanon and Syria. The working hypothesis is that by identifying devices in the proximity of trade-associated point of interests, such as border checkpoints, and mapping their mobility traces throughout the time horizon, the movement captured may indicate changes in trade patterns and potentially identify new trade centers and corridors.

The results presented are the stay locations (or “stops”) generated by sub-panels composed of (A) devices seen at 6 border checkpoints and (B) devices seen in the proximity of border, except the 6 checkpoints. The sub-panels aim to estimate formal and informal trade activities, respectively.

```{seealso}
**The results are aggregated on governorate-level and on H3 tiles for a more granular outlook and are available as interactive maps**
