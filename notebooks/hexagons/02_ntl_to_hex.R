...
# Load data --------------------------------------------------------------------
for(res_i in 4:6){
  gadm_sf <- readRDS(file.path(hex_dir, "RawData", paste0("hex_", res_i, ".Rds")))
  
  gadm_df <- gadm_sf
  gadm_df$geometry <- NULL
  
  gadm_sp <- gadm_sf %>% as("Spatial")
  ...
  saveRDS(data_df, file.path(hex_dir, "FinalData", paste0("hex_", res_i, "_ntl.Rds")))
}
