...
def create_port_calls(data, date0, date1, port, country):
    ...
    merged_time_series_first_select_2 = first_day[
        [
            "mmsi",
            "vessel_type",
            "Date",
    ...
    merged_time_series_first_select_2["Date"] = merged_time_series_first_select_2[
        "Date"
    ].astype(str)
    merged_time_series_first_select_2.rename(
        columns={
            "Date": "date-entry",
            "draught": "draught-in",
            "heading": "heading-in",
    ...
    merged_time_series_last_select_2 = last_day[
        ["mmsi", "Date", "dtg", "draught", "heading"]
    ].copy()

    #### add full date list per mmsi
    merged_time_series_first_new = merged_time_series_first_select_1.set_index(
        ["mmsi", "Date"]
    ).unstack(level=0)
    merged_time_series_first_new = pd.concat(
        [merged_time_series_first_new, list_dates_df], axis=1
    )
    ...
    #### add full date list per mmsi
    merged_time_series_last_new = merged_time_series_last_select_1.set_index(
        ["mmsi", "Date"]
    ).unstack(level=0)
    merged_time_series_last_new = pd.concat(
        [merged_time_series_last_new, list_dates_df], axis=1
    )
    ...
    turn_around_time = (
        time_diff_new.unstack().dropna().reset_index(level=["mmsi", "Date"])
    )
    turn_around_time = pd.merge(
        turn_around_time, merged_time_series_last_select_2, on=["Date", "mmsi"]
    )
    turn_around_time.rename(
        columns={
            "draught": "draught-out",
            "dtg": "datetime-leave",
            0: "turn_around_time",
    ...
    turn_around_time["datetime-entry"] = turn_around_time.apply(
        lambda row: substract_seconds(row["datetime-leave"], row["seconds"]),
        axis=1,
        result_type="reduce",
    )
    turn_around_time["datetime-entry"] = turn_around_time["datetime-entry"].dt.round(
        "1s"
    )
    ...
    port_calls = pd.merge(
        turn_around_time, merged_time_series_first_select_2, on=["date-entry", "mmsi"]
    )
